...
def run():
    ...
    for i, entry in enumerate(collection):
        ...
        if len(entire_text) < 100:
            ...
            try:
                ...
                if not os.path.exists("extracted_info/schema_0shot_simple"):
                    os.makedirs("extracted_info/schema_0shot_simple")

                # # # saving the information to a file
                with open(
                        f"extracted_info/schema_0shot_simple/{titles_to_dois[entry['title']].replace('/', '.')}_extracted_info.json",
                        "w") as f:
    ...
    if len(failed) > 0:
        with open("extracted_info/schema_0shot_simple/failed_DOIs.json", "a") as f:
