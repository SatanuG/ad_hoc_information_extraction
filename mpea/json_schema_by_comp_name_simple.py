    ...
    for i, entry in enumerate(collection):
        ...
        if len(entire_text) < 100:
            ...
            try:
                ...
                if not os.path.exists("./extracted_info_jsons/schema_0shot_simple"):
                    os.makedirs("extracted_info_jsons/schema_0shot_simple")
                # # # saving the information to a file
                with open(
                        f"./extracted_info/schema_0shot_simple/{entry['doi'].replace('/', '.')}_extracted_info.json",
                        "w") as f:
